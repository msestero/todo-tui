...
- **Reuse over respawn.** If the row already has a session id, before launching anything new we look for the live tmux artifact: inside tmux we `find_window(text)` and `select-window` if it's there; outside tmux we `has-session todo-<id>` and `attach`. Only when there's no existing window/session do we fall through to a fresh `launcher.launch`. Note: a reused window keeps the original system-prompt context — checking off more subtasks won't re-inject until the window is closed and `c` is pressed again.
